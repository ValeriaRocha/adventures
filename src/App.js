...
//para desplegar las posibles islas destino
  islasDestino(){
    let newJSX = [];
    this.state.mapa[this.state.currentIsland].map((isla, index) => {
      ...
      if(isla == "0"){
        ...
        nombreIsla = this.state.islas[isla].isla;
      }

      newJSX.push( <div key={index} className="sectionIslaDestino1"> <Isla nombre={nombreIsla}
        texto={this.state.islas[isla].texto}
        onClick={() => this.navigate(isla)}
        imagen={this.state.islas[isla].imagen} /> </div>);
      })
  ...
  return (
    <div>
      <div className="superior">
        ...
        <h1> {this.state.islas[indice].isla} </h1>
        <img className="imgIsla" src={"/images/" + this.state.islas[indice].imagen} alt="isla" />
      </div>
      <div className="inferior">
        <p>{this.state.islas[indice].textoLlegada}</p>
